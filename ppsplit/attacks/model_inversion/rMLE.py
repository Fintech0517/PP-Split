...
class rMLE():
    ...
    def inverse(self,client_net,
        ...
        infer_time_list = []
        mseloss_list = []
        for i, (trn_X, trn_y) in enumerate(tqdm.tqdm(dataloader)):  # 对dataloader遍历
            ...
            for j in range(Niters):
                ...
                smashed_data_fake = client_net(originData)
                mseloss = ((smashed_data_fake-smashed_data_raw) ** 2).mean()

                mseloss_single_list.append(mseloss.cpu().numpy())
            ...
            if self.data_type == 0: # 表格数据
                sim_metrics._collect_sim_tab(originData,inverted_data)
                X_fake_list.append(inverted_data.cpu().detach().squeeze().numpy())
            elif self.data_type == 1: # 图像数据
                sim_metrics._collect_sim_img(originData,inverted_data, deprocess)
                # 保存图片
