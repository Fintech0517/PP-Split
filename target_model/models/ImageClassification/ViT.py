...
class Encoder(nn.Module):
    ...
    def __init__(
        self,
        split_layer, # for 16B 模型 # 但是一定是要有输入的，不能为None
        seq_length: int,
        num_layers: int,
        num_heads: int,
        hidden_dim: int,
        ...
        encoder_nums = split_layer if split_layer < num_layers else num_layers
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # split 0
        if self.split_layer > 0: # 否则下面都不要了
            seq_length = (image_size // patch_size) ** 2

            # Add a class token # 这是啥？
            self.class_token = nn.Parameter(torch.zeros(1, 1, hidden_dim))
            seq_length += 1

            # encoder 模块，正经transformer
            self.encoder = Encoder(
                self.split_layer, # 位置参数
                seq_length,
                num_layers, 
                num_heads,
                hidden_dim,
        ...
        if self.split_layer > num_layers: # 12是encoder的最后一层
            # 第三块，head
            heads_layers: OrderedDict[str, nn.Module] = OrderedDict()
            if representation_size is None: # 一般是这个
...
def vit_b_16(*, weights= None, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_b_32(*, weights= None, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_16(*, weights= None, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_l_32(*, weights= None, **kwargs: Any) -> VisionTransformer:
    """
...
def vit_h_14(*, weights= None, **kwargs: Any) -> VisionTransformer:
    """
